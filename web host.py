...
#FrogBoard Forum Software version 0.4
				...
				#post viewer
					...
					if posts >= rid:
						with open('D:\db\post\post'+postid+'.post') as postraw:
							post = json.load(postraw)
							postraw.close()
						page = posttop + post["credate"] + post["creator"] + post["id"] + post["post"] + postbottom
					...
					with open('D:\db\pnumb.db', 'r+') as idfile:
						idfile.write(saveid)
						idfile.close()
					#create a new json table for us to put our post contents into
					rawjson = {}
					#define json table contents
					rawjson["creator"] = "test"
					rawjson["credate"] = datetime.now().strftime('%m-%d-%Y %H:%M:%S')
					rawjson["id"] = saveid
					rawjson["post"] = post
					#make that json into a string
					jsonpost = json.dumps(rawjson)
					with open('D:\db\post\post'+saveid+'.post', 'w') as savepost:
						print 'saving post...'
						savepost.write(jsonpost)
						print 'post saved!'
						savepost.close()
